...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  p->switch_count = 0;
  p->burst_time = 100;	//default burst time is set to 10;
  return p;
}
...
scheduler(void)
{
  ...
  for(;;){
	...
	if( p1->burst_time < highP->burst_time){
      ...
      p->switch_count = p->switch_count + 1;	//p->switch_count = p->switch_count + 1;	
      swtch(&(c->scheduler), p->context);		//replace highP by p;
      switchkvm();
...
//this function will return number of currently running processes
int
numberOfRunningProcess(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
     if(p->state != UNUSED){
        count++;
	//whenever state of process p is active than we are incrementing the count.
     }
  }
...
getMaxPid(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
     if(p->pid > mx && p->state != UNUSED){
        mx = p->pid;
	//whenever state of process p is active than we are checking the current max pid and updating it.
     }
  }
